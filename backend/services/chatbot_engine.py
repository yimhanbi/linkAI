...
class ChatbotEngine:
    ...
    async def initialize(self):
        # Ensure chat history indexes exist
        ...
        try:
            ...
            # 1. 인덱스 초기화 및 와일드카드 재생성
            self.logger.info("Rebuilding text index for collection: %s", self.mongo_collection_name)
            try:
                # 기존에 꼬여있을 수 있는 모든 인덱스 삭제 (단, _id 인덱스는 제외됨)
                await collection.drop_indexes()
                
                # 정확한 문법($**)으로 다시 생성
                await collection.create_index(
                    [("$**", "text")],
                    name="patent_all_fields_text_index",
                    default_language="none"  # 한국어 이름/고유명사 검색을 위해 분석기 간섭 최소화
                )
